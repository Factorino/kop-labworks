...
Стек приложения (`docker/docker-compose.yml`) запускается в одном из двух
режимов. Режим — первый аргумент команды, по умолчанию `app`; всё после него
передаётся docker compose как есть: имена сервисов и флаги.

| Режим | Что запускается | Для чего |
| --- | --- | --- |
| `app` | Инфраструктура и процессы приложения в контейнерах; порты инфраструктуры наружу не публикуются | Запуск всего приложения |
| `dev` | Только инфраструктура, порты на `127.0.0.1` (`docker/docker-compose.dev.yml`) | Приложение запускается на хосте |

Оба режима — один compose-проект: контейнеры и данные общие, переключение
пересоздаёт только изменившиеся контейнеры.

| Команда | Что делает |
| --- | --- |
| `just up [app\|dev] [сервис...]` | Запуск в фоне с ожиданием готовности: `just up dev`, `just up app postgresql` |
| `just stop`, `just restart` | Остановка и перезапуск: `just restart dev rabbitmq` |
| `just logs`, `just ps` | Журналы и список контейнеров: `just logs dev redis` |
| `just down [app\|dev]` | Удаление контейнеров; тома сохраняются |
...
| `just test-container` | Тесты в образе рядом с временными PostgreSQL, Redis и RabbitMQ; после прогона всё удаляется |
...
хосте нужен `just up dev`; `just test-container` поднимает свои временные
копии сам и не зависит ни от `.config/`, ни от данных стека.
